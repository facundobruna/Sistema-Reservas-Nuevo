...
## Separación de backend y frontend (previo al TP5)

### Por qué se hizo

El TP2 pedía **dos Dockerfiles: uno para el backend y otro para el frontend**. Yo entregué uno solo
y lo justifiqué (ver *Decisiones de contenerización*, TP2): la app era un monolito Next.js donde el
App Router servía las páginas y `src/app/api/**` era la API, todo compilando al mismo artefacto.
Dos Dockerfiles para un único artefacto habrían sido dos copias del mismo archivo.

En la defensa del P1 el profesor me dijo que **a partir del TP5 necesita el backend separado del
frontend**. Así que lo separé antes de empezar el TP5, y no durante: meter una reestructuración de
todo el repositorio en la misma entrega en la que hay que demostrar un ciclo rojo→verde de tests es
pedirle a una sola entrega que soporte dos cambios grandes.

### Qué opciones evalué, y por qué elegí la que elegí

Había dos formas de separar:

**A. Sacar la API a un servidor Node propio** (Express o Fastify), dejando el frontend en Next.

**B. Dos aplicaciones Next independientes**: una que expone solo `/api/v1/**` y no tiene ninguna
pantalla, y otra que tiene solo pantallas y no toca la base.

Antes de decidir conté el acoplamiento real, en vez de estimarlo:

| Medición | Resultado |
|---|---|
| Funciones handler a migrar | **65** (25 `GET`, 23 `POST`, 10 `PATCH`, 7 `DELETE`) |
| Archivos `route.ts` | **49** |
| De esos, los que importan `NextResponse` de `next/server` | **49 de 49** |
| Módulos que leen la cookie con `cookies()` de `next/headers` | 3 (staff, comensal, superadmin) |
| Archivos del frontend que tocaban la base | **7** |

La opción A obligaba a reescribir las 65 funciones a mano (`NextResponse.json(x, {status})` →
`res.status().json()`), rehacer el ruteo de los segmentos dinámicos (`[slug]`, `[id]`) y reemplazar
los 3 módulos de sesión, con 3 archivos de test como única red de seguridad. La opción B mueve los
65 handlers **sin tocar una línea**: lo que cambia es dónde vive el archivo, no lo que dice.

Elegí **B**. El criterio: lo que el profesor pidió —y lo que vale para el resto de la materia— es
que haya dos cosas desplegables por separado, con su propio Dockerfile, su propia imagen y su propio
contenedor, hablando entre sí por HTTP. Eso lo da B, sin poner en riesgo una aplicación que ya
funciona una semana antes del TP5. Reescribir 65 funciones habría sido cambiar el framework del
backend, que es una decisión distinta de "separar el backend del frontend" y que nadie me pidió.

Lo que la medición también mostró es que el corte ya estaba limpio: fuera de la API, **solo 7
archivos** tocaban la base. No hubo que desenredar nada — hubo que mover.
...
`docker compose` pasó de 3 servicios a **4**: `db`, `migrate`, `backend`, `frontend`. Son tres
imágenes publicables (`sistema-reservas-backend`, `sistema-reservas-frontend`,
`sistema-reservas-migrate`), y el pipeline de CI pasó de dos jobs a **tres**, uno por imagen, cada
uno con su propio `scope` de cache.

La versión nueva es **v0.3.0**. Las imágenes de v0.2.0 siguen existiendo en el registry y siguen
funcionando: una imagen publicada no se pisa, se publica una versión nueva al lado.

### Por qué el frontend reenvía `/api` en vez de que el navegador le pegue directo al backend

El frontend declara un *rewrite* en `next.config.ts`: todo lo que empieza con `/api` lo reenvía a
`http://backend:3000`. Desde el navegador hay un único origen.

El motivo es la sesión. El login devuelve una cookie `httpOnly` firmada. Si el navegador hablara
directo con el backend, esa cookie sería de un origen distinto (alcanza con que cambie el puerto) y
haría falta: CORS con `credentials`, `SameSite=None` en la cookie y, como `SameSite=None` obliga a
`Secure`, **HTTPS** — que en local no hay. Serían tres cosas nuevas que pueden fallar, para resolver
un problema que el reenvío hace desaparecer.

Hay una segunda razón, que es la que importa para los TPs que vienen: el backend no queda expuesto.
En compose el puerto 3100 está publicado solo para poder probarlo con curl; en un despliegue real
se saca y el único que puede alcanzar la API es el frontend, por la red interna.

### Por qué el frontend no valida la sesión por su cuenta

Antes, la guarda de las pantallas del panel (`requireStaffPage`) abría la cookie firmada ahí mismo,
porque el proceso era uno solo y tenía `AUTH_SECRET`.

Ahora el secreto de firma vive **únicamente en el backend**. El frontend no puede abrir el token ni
fabricar uno: lo reenvía tal cual a `GET /api/v1/auth/staff/me` y el backend contesta quién es o
responde 401. El contenedor del frontend no tiene `AUTH_SECRET` ni `DATABASE_URL` — si alguien
entrara a esa imagen no encontraría con qué firmar una sesión ni con qué conectarse a la base.

Endpoints nuevos que hicieron falta, todos por la misma razón (la información o la credencial vive
del lado del backend):

| Endpoint | Para qué |
|---|---|
| `GET /api/v1/auth/staff/me` | quién es el usuario de la cookie de staff |
| `GET /api/v1/auth/superadmin/me` | ídem, para la sesión de superadmin |
| `GET /api/v1/admin/panel-access` | nombre del restaurante + suscripción + **veredicto** de acceso |
| `GET /api/v1/r/{slug}/reservations/{id}/cancel` | datos de la pantalla de cancelación, autenticada por el token firmado del email |

### La regla de negocio se quedó del lado del backend

El caso de `panel-access` es el que mejor muestra el criterio. El layout del panel decidía el
bloqueo así: leía el restaurante, leía la suscripción y llamaba a `evaluatePanelAccess(...)`.

Podría haber expuesto un endpoint que devolviera el restaurante y la suscripción, y dejar que el
frontend siguiera llamando a `evaluatePanelAccess`. No lo hice: eso habría obligado a tener una
copia de la regla —y de los tipos de la suscripción— de los dos lados. El endpoint devuelve el
veredicto ya calculado (`ok` / `trial_expired` / `payment_required` / `suspended`) y el frontend
solo decide a dónde redirigir. **La regla sigue viviendo en un solo archivo**
(`backend/src/db/subscription.ts`), que es lo que decía la nota de arquitectura desde el TP2.

### La única duplicación deliberada: la validación de teléfono

`validation/phone.ts` quedó en los dos lados, y es a propósito. El frontend valida el formato para
avisarle a la persona antes de enviar el formulario (eso es UX); el backend vuelve a validar porque
no puede confiar en lo que le llega (eso es seguridad). Si borrara la del frontend, el formulario
avisaría del error recién después del viaje al servidor; si borrara la del backend, cualquiera con
curl podría guardar un teléfono inválido. Son dos validaciones con dos propósitos distintos que
casualmente miran lo mismo.

### Problemas encontrados y cómo los solucioné

1. **Los tipos del frontend mentían, y la separación lo destapó.** Las pantallas del panel derivaban
   sus tipos del esquema de Drizzle (`typeof zone.$inferSelect`). Eso arrastraba el ORM al frontend,
   pero además era **incorrecto**: esos tipos decían `createdAt: Date`, y el dato ya venía de un
   `fetch`, o sea que en runtime siempre fue un string ISO — JSON no tiene tipo fecha. El bug estaba
   dormido porque casi ningún componente hacía cuentas con esas fechas. El que sí lo hacía era
   `billing-content.tsx`, con `DateTime.fromJSDate(subscription.trialEndsAt)`; funcionaba solo
   porque ese dato llegaba como prop de un Server Component, que es el único camino por el que Next
   preserva un `Date` de verdad. Al separar, ese dato pasó a llegar por HTTP y el `fromJSDate` se
   habría roto. Escribí los tipos a mano en `frontend/src/lib/api/types.ts`, con los timestamps como
   `string`, y cambié la conversión a `DateTime.fromISO`. Ahora el frontend depende del **contrato
   HTTP**, no de las tablas.

2. **La pantalla de cancelación no tenía de dónde agarrarse.** Se llega desde el link de un email,
   sin sesión: la credencial es un token firmado en la URL. Verificarlo necesita `AUTH_SECRET`, que
   ya no está en el frontend. La API tenía el `POST` que cancela, pero nada para *leer* la reserva y
   pintar la pantalla de confirmación. Agregué un `GET` al mismo endpoint, que verifica el token y
   devuelve solo lo que la pantalla muestra. Mantuve la distinción que ya existía: **leer es GET,
   cancelar sigue siendo POST**, porque un escáner de links de email no puede cancelar una reserva
   real con solo abrir la URL.

3. **El `.gitignore` dejó de cubrir lo que cubría.** Los patrones estaban escritos con barra inicial
   (`/node_modules`, `/.next/`), que en git significa "solo en la raíz del repositorio". Con el
   código en `backend/` y `frontend/`, `backend/node_modules` y `frontend/.next` quedaron sin
   ignorar — y eso son decenas de miles de archivos a punto de entrar a un commit. Lo vi en el
   `git status` antes de agregar nada. Saqué las barras iniciales para que los patrones valgan en
   cualquier nivel.

4. **Un endpoint que existía me devolvía 404.** Agregué `/api/v1/auth/staff/me`, lo vi listado en la
   salida del build, y al probarlo me daba 404 — mientras que los endpoints viejos andaban. Estuve
   un rato buscando el error en el código. No estaba en el código: había quedado un proceso del
   backend anterior ocupando el puerto 3000, el proceso nuevo falló al arrancar con `EADDRINUSE` y
   yo le estaba pegando al build viejo, que efectivamente no tenía ese endpoint. El log del arranque
   lo decía desde el principio; yo estaba mirando la respuesta de curl en vez del log del servidor.
...
demo cargado, y recorrí el sistema de punta a punta:

| Qué probé | Resultado |
|---|---|
| `/api/v1/health` contra el backend directo | `{"status":"ok","database":"up"}` |
| `/api/v1/health` a través del frontend (prueba el reenvío) | idéntico |
| Login de staff por el reenvío | 200, y el `Set-Cookie` llega al navegador con el origen del frontend |
| `/admin/demo` **sin** sesión | 307 al login |
| `/admin/demo` **con** sesión | 200, con el nombre del restaurante y el email del usuario |
| `/admin/demo/billing` | 200, leyendo la suscripción por `panel-access` |
| Reserva completa (consultar disponibilidad + `POST`) | 201, y la fila quedó `confirmed` en Postgres |
| Pantalla de cancelación con token válido / inválido | 200 con fecha y hora bien convertidas a la zona horaria / 404 |
| `/r/no-existe` | 404 |
| Suite de tests del backend | 18 tests, 3 archivos, todos en verde |
| Búsqueda de residuos: `@/db` o `next/server` en el frontend | ninguno |
| Búsqueda de residuos: componentes o librerías de UI en el backend | ninguno |

### Declaración de uso de IA

Usé **Claude (Cowork)** para medir el acoplamiento real antes de decidir (los números de la tabla de
opciones salen de contar los archivos, no de estimar), para ejecutar el movimiento de archivos con
`git mv` conservando el historial, para escribir los dos `Dockerfile`, el `docker-compose.yml` de 4
servicios y los tipos del contrato HTTP, y para redactar el borrador de esta sección.

**Cómo lo verifiqué:**

- La decisión entre A y B la tomé yo, y en contra de la primera recomendación que había recibido:
  la opción A me la habían propuesto como "la mejor si tenés un fin de semana". Al ver el número
  real de handlers acoplados a `next/server` (49 de 49) preferí no cambiar el framework del backend
  a una semana del TP5.
- Las pruebas de la tabla de verificación se corrieron de verdad, contra Postgres, con el seed
  cargado — no son una lista de intenciones. La reserva de prueba la confirmé consultando la tabla
  `reservation` directamente con `psql`, no leyendo la respuesta del endpoint.
- El bug de `Date` vs string ISO lo encontré revisando por qué el tipo derivado del esquema no podía
  viajar por HTTP; no estaba en el plan de trabajo.
