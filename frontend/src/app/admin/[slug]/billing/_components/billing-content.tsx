...
import type { PanelAccess as Access, Subscription } from "@/lib/api/types";
...
function StatusBlock({ access, subscription }: { access: Access; subscription: Subscription | null }) {
  ...
  if (subscription?.status === "trialing" && subscription.trialEndsAt) {
    const daysLeft = Math.max(0, Math.ceil(DateTime.fromISO(subscription.trialEndsAt).diffNow("days").days));
    return (
      <p className="text-sm text-muted-foreground">
